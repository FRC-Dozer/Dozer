 ...
 *
 * <p>
 * A Generator is a function which returns an iterator. It looks like a normal function except that
 * it contains yield statements for producing a series a values usable in a for-loop or that can be
 * retrieved one at a time with the next() function. Each "yield" temporarily suspends processing,
 * remembering the location execution state (including local variables and pending try-statements).
 * When the generator resumes, it picks-up where it left-off (in contrast to functions which start
